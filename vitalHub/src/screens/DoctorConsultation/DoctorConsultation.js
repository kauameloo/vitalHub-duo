...
export const DoctorConsultation = ({ navigation }) => {
  ...
  async function BuscarFotoDePerfil() {
    ...
    await api
      .get(`/Usuario/BuscarPorId?&id=${tokenB.idUsuario}`)
      .then((response) => {
  ...
  return (
    <Container>
      ...
      <ButtonHomeContainer>
        ...
        <FilterButton
          onPress={() => {
            setSelected("Realizadas");
          }}
          selected={selected === "Realizadas" ? true : false}
          text={"Realizadas"}
        />

        <FilterButton
          onPress={() => {
            setSelected("Canceladas");
          }}
          selected={selected === "Canceladas" ? true : false}
          text={"Canceladas"}
        />
      </ButtonHomeContainer>
        ...
        renderItem={({ item }) =>
          ...
          item.situacao.situacao == selected && (
            <Card
              ...
              // onPressAppointmentCard={() => setShowModalAppointment(item.situacao.situacao === 'Agendadas' ? true : false)}
