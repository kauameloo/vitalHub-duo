...
export const ViewPrescription = ({ navigation, route }) => {
  ...
  async function BuscarProntuario() {
    if (idConsulta != null) {
      ...
      await api
        .get(`/Consultas/BuscarPorId?id=${route.params.consulta.id}`)
        .then((response) => {
          setConsultaSelecionada(response.data);
          console.log(`TESTEEEEEE ` + consultaSelecionada);
        })
        .catch((error) => {
          console.log(error);
        });
      ...
      <ScrollContainer>
        {consultaSelecionada != null ? (
          <Container>
            <ViewImage
              source={{
                uri: consultaSelecionada.medicoClinica.medico.idNavigation.foto,
              }}
            />
            ...
            <BoxDescription>
              ...
              />
              <DescriptionDoc
                description={`CRM-${consultaSelecionada.medicoClinica.medico.crm}`}
              />
            </BoxDescription>
            ...
            <BoxBtn>
              <SendButton
                onPress={() => {
                  navigation.navigate("Camera", { id: consultaSelecionada.id });
                }}
                text={"Enviar"}
              />
              <CardCancel
